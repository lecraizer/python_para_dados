...
while p1.esta_vivo() and p2.esta_vivo():
    ...
    jogadores = random.sample([p1, p2], k=2)
    for atacante, alvo in ((jogadores[0], jogadores[1]), (jogadores[1], jogadores[0])):
        ...
        logs.append({"turno": turno, "atacante": atacante.nome, "alvo": alvo.nome, "dano": dano})

        # A inclusão dessas próximas 2 linha basicamente interrompe o jogo se alvo morreu no ataque de atacante
        if not alvo.esta_vivo():
            break


    print(f"{p1.nome} -> Vida: {p1.vida:.2f} | Mana: {p1.mana}")
    print(f"{p2.nome} -> Vida: {p2.vida:.2f} | Mana: {p2.mana}")

    turno += 1
    time.sleep(0.8)

print("\n--- FIM DE JOGO ---")
vencedor = p1.nome if p1.esta_vivo() and not p2.esta_vivo() else (p2.nome if p2.esta_vivo() else "Empate")
